...
for (target_window in windows_list){
  ...
  for (i in 1:nrow(onts_list)) {
    ...
    counts_file <- paste(
      countDir, "/counts/", sample_name, "_", target_chr, ".txt",
      sep=""
    )
    ...
    data <- data[, metrics := paste("r=", total_counts, ", c=", total_cells, sep="")]
  ...
  out_stem <- paste(sample_name,target_window, sep="_")
  out_file <- paste(outDir, out_stem, ".plotterFile", sep="")
